...
Build a single, self-contained Obsidian plugin called **Olen** that replaces the current fragmented system (separate `Home.md` dataviewjs, `Drawing hub.md`(template), `Drawing Session.md`(and the other session files), into **one cohesive app-like experience** on Obsidian Mobile that communicates to the `TrackHabitRank` main.js plugin).
...
### Screen 5: Set Your Tracking Style

Configure how Olen categorizes and reflects on your session completions.

Session Completion Categories

Olen uses three internal states to track how a session was completed. These can be enabled, disabled, or renamed:

Default Name	Purpose	Behavior	
Discipline	Completed through willpower, not enjoyment	Logs damage to boss, standard XP, logs stoic/serious quote	
Flow	Completed in effortless absorption	Bonus XP, logs aspirational/inspirational quote	
Skipped	Avoided or abandoned	No XP, possible streak break, logs warning/reminder quote	

Configuration Options:

- Enable/Disable States: Toggle each category on or off. Minimum one must remain enabled. If only one is enabled, the completion modal skips the selection step and automatically assigns that state.
- Rename States: Change "Discipline" to "Grind," "Flow" to "Zone," "Skipped" to "Missed"—or any terminology matching your philosophy.
- Visual Indicators: Assign unique icons (◆ ≈ ○ by default) and colors to each state.

---

Quote System Configuration

For each enabled completion state, configure an associated quote pool that appears in the session finish modal:

Quote Source: Select a vault folder containing quote files (e.g., `Quotes/Discipline/`, `Philosophy/`, `Journal/Reflections/`)

Quote Format: Specify how individual quotes are identified within files:

Format	Description	Detection Rule	
One per file	Each markdown file contains one complete quote	Entire file content = one quote	
Paragraph blocks	Multiple quotes separated by blank lines	Text block between empty lines = one quote	
Bullet list	Lines starting with `-` or `*`	Line beginning with `- ` or `* ` = one quote	
Quoted strings	Text wrapped in `"..."` or `'...'`	Content between matching quote marks = one quote	
Horizontal rule	Quotes separated by `---` or `***`	Content between `---` lines = one quote	
Custom delimiter	User-defined separator pattern	Regex pattern input (advanced)	

Quote Parsing Rules:
- Attribution detection: If a quote contains `—` (em-dash) or `-` at the end, split into `quote text` and `attribution` for styled display
- Max length: Truncate quotes exceeding N characters (configurable)
- Randomization: Shuffle order or use chronological/alphabetical sequence
- No repeat: Track recently shown quotes to avoid immediate repetition
...
Discipline State:
  ...
  → Format: One per file
  → Icon: ◆
  → Color: #928d85 (muted gold)

Flow State:
  → Folder: Journal/FlowStates.md
  → Format: Horizontal rule separated
  → Icon: ≈
  → Color: #c9a84c (burnished gold)

Skipped State:
  → Folder: Quotes/Motivation/
  → Format: Bullet list
  → Icon: ○
  → Color: #8b2d35 (crimson)
```

Fallback Behavior: If a state's configured folder is empty or missing, Olen uses generic hardcoded quotes for that state until the user updates the configuration.
...
About the calendar: I uploaded a calendar plugin in the repo inside the folder "Calendar plugin" make the system communicate with it.